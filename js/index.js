...
let availIndexes = updateAvailIdexes();
...
matrixVals = getCurrentMatrixValues();
availIndexes = updateAvailIdexes();
updateColors();

let check = checkMatrixEquality(prewMatrix, matrixVals);

if(availIndexes.length === 0 && check === true){
 gameOver("Loose")
}

if(move % moveFactor === 0) {
generateNewBlock();
}
...
function moveRigth(row){
    shiftRight(row);

    for(let i = 2; i>= 0; i--){
        let currElement = row[1].firstElementChild;
